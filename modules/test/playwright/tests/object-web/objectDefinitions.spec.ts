...
test.describe('Manage object definitions through Model Builder', () => {
	...
	test('can create an object definition by model builder', async ({
		...
		await expect(
			modelBuilderLeftSidebarPage.leftSidebarItems.filter({
				hasText: objectDefinition.label['en_US'],
			})
		).toBeVisible();
	});
	...
	test('can delete an object definition by model builder leftsidebar', async ({
		...
		await modelBuilderLeftSidebarPage.clickLeftSideBarItem(
			objectDefinition1.label['en_US']
		);

		await modelBuilderLeftSidebarPage.clickObjectDefinitionActionsButtonInLeftSidebar(
			objectDefinition1.label['en_US']
		);
	...
	test('linked object definitions are created when object definitions are related and put into different folders', async ({
		...
		await modelBuilderLeftSidebarPage.leftSidebarItems
	...
	test('see object definition details', async ({
		...
		for (const objectDefinition of [department, employee]) {
			...
			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionLabel
			).toHaveValue(objectDefinition.label['en_US']);

			await modelBuilderRightSidebarPage.rightSidebarObjectDefinitionLabelLocalizationButton.click();
			...
			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionPluralLabel
			).toHaveValue(objectDefinition.pluralLabel['pt_BR']);

			await modelBuilderRightSidebarPage.rightSidebarObjectDefinitionPluralLabelLocalizationButton.click();
			...
			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionActivateObject
			).toBeChecked({checked: objectDefinition.active});

			// Entry Display Container

			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionEntryTitleField
			...
			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionScope
			).toHaveText(objectDefinition.scope, {ignoreCase: true});

			const [_, panelLink] = objectDefinition.panelCategoryKey.split('.');

			await expect(
				modelBuilderRightSidebarPage.rightSidebarObjectDefinitionPanelLink
